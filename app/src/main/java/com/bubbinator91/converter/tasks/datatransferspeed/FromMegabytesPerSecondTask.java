...
/**
 * An implementation of {@link AsyncTask} that will handle the conversion from megabytes per second
 ...
 * round to.
 */
public class FromMegabytesPerSecondTask
        extends AsyncTask<String, Void, Pair<List<String>, ConversionErrorCodes>> {
    protected final String TAG = FromMegabytesPerSecondTask.class.getSimpleName();
    ...
    protected Pair<List<String>, ConversionErrorCodes> doInBackground(String... params) {
        ...
        try {
            return MegabytesPerSecond.toAll(params[0], Integer.parseInt(params[1]));
