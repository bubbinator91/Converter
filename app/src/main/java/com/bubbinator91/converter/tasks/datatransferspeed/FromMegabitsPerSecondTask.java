...
/**
 * An implementation of {@link AsyncTask} that will handle the conversion from megabits per second
 ...
 * round to.
 */
public class FromMegabitsPerSecondTask
        extends AsyncTask<String, Void, Pair<List<String>, ConversionErrorCodes>> {
    protected final String TAG = FromMegabitsPerSecondTask.class.getSimpleName();
    ...
    protected Pair<List<String>, ConversionErrorCodes> doInBackground(String... params) {
        ...
        try {
            return MegabitsPerSecond.toAll(params[0], Integer.parseInt(params[1]));
