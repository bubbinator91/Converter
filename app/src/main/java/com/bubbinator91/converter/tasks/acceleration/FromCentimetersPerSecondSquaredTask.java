...
/**
 * An implementation of {@link AsyncTask} that will handle the conversion from centimeters per
 * second squared to the other acceleration values in a separate thread.
 *
 ...
 * round to.
 */
public class FromCentimetersPerSecondSquaredTask
        extends AsyncTask<String, Void, Pair<List<String>, Integer>> {
    protected final String TAG = FromCentimetersPerSecondSquaredTask.class.getSimpleName();
    ...
    protected Pair<List<String>, Integer> doInBackground(String... params) {
        ...
        try {
            return CentimetersPerSecondSquared.toAll(params[0], Integer.parseInt(params[1]));
