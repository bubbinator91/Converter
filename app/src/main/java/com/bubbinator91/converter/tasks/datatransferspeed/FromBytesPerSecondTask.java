...
/**
 * An implementation of {@link AsyncTask} that will handle the conversion from bytes per second to
 * the other data transfer speed values in a separate thread.
 *
 ...
 * round to.
 */
public class FromBytesPerSecondTask
        extends AsyncTask<String, Void, Pair<List<String>, Integer>> {
    protected final String TAG = FromBytesPerSecondTask.class.getSimpleName();
    ...
    protected Pair<List<String>, Integer> doInBackground(String... params) {
        ...
        try {
            return BytesPerSecond.toAll(params[0], Integer.parseInt(params[1]));
