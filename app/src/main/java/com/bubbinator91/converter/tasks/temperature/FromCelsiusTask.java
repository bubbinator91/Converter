...
/**
 * An implementation of {@link AsyncTask} that will handle the conversion from celsius to the other
 * temperature values in a separate thread.
 *
 ...
 * round to.
 */
public class FromCelsiusTask
        extends AsyncTask<String, Void, Pair<List<String>, Integer>> {
    protected final String TAG = FromCelsiusTask.class.getSimpleName();
    ...
    protected Pair<List<String>, Integer> doInBackground(String... params) {
        ...
        try {
            return Celsius.toAll(params[0], Integer.parseInt(params[1]));
