...
/**
 * An implementation of {@link AsyncTask} that will handle the conversion from gigabytes per second
 ...
 * round to.
 */
public class FromGigabytesPerSecondTask
        extends AsyncTask<String, Void, Pair<List<String>, Integer>> {
    protected final String TAG = FromGigabytesPerSecondTask.class.getSimpleName();
    ...
    protected Pair<List<String>, Integer> doInBackground(String... params) {
        ...
        try {
            return GigabytesPerSecond.toAll(params[0], Integer.parseInt(params[1]));
