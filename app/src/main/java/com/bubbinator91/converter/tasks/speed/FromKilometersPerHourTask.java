...
/**
 * An implementation of {@link AsyncTask} that will handle the conversion from kilometers per hour
 * to the other speed values in a separate thread.
 *
 ...
 * round to.
 */
public class FromKilometersPerHourTask
        extends AsyncTask<String, Void, Pair<List<String>, Integer>> {
    protected final String TAG = FromKilometersPerHourTask.class.getSimpleName();
    ...
    protected Pair<List<String>, Integer> doInBackground(String... params) {
        ...
        try {
            return KilometersPerHour.toAll(params[0], Integer.parseInt(params[1]));
