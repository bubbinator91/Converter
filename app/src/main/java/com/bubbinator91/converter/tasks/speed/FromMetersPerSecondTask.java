...
/**
 * An implementation of {@link AsyncTask} that will handle the conversion from meters per second to
 ...
 * round to.
 */
public class FromMetersPerSecondTask
        extends AsyncTask<String, Void, Pair<List<String>, ConversionErrorCodes>> {
    protected final String TAG = FromMetersPerSecondTask.class.getSimpleName();
    ...
    protected Pair<List<String>, ConversionErrorCodes> doInBackground(String... params) {
        ...
        try {
            return MetersPerSecond.toAll(params[0], Integer.parseInt(params[1]));
