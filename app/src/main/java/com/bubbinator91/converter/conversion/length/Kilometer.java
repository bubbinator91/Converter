...
public class Kilometer {
    private static final String TAG = "Kilometer";

    /**
     * Static method that takes in the kilometer value as a {@link String} and converts it to
     * inches, feet, yards, miles, millimeters, centimeters, and meters.
     *
     ...
     *
     * @return  The equivalent inch, foot, yard, mile, millimeter, centimeter, and meter values
     *          stored in an {@link ArrayList}, in that order.
     */
    public static ArrayList<String> toAll(@NonNull String kilometer, int roundingLength) {
        Timber.tag(TAG + ".toAll").i("Entered");
        Timber.tag(TAG + ".toAll").i("kilometer = " + kilometer);
        int decimalPlaces = (roundingLength < 0) ? 0 : roundingLength;
        ArrayList<String> results = new ArrayList<>();
        if (Utils.isNumeric(kilometer)) {
            try {
                results.add(toInch(kilometer, decimalPlaces));
                results.add(toFoot(kilometer, decimalPlaces));
                results.add(toYard(kilometer, decimalPlaces));
                results.add(toMile(kilometer, decimalPlaces));
                results.add(toMillimeter(kilometer, decimalPlaces));
                results.add(toCentimeter(kilometer, decimalPlaces));
                results.add(toMeter(kilometer, decimalPlaces));
     ...
     */
    public static String toInch(@NonNull String kilometer, int roundingLength)
            throws NumberFormatException {
        Timber.tag(TAG + ".toInch").i("Entered");
        int decimalPlaces = (roundingLength < 0) ? 0 : roundingLength;
        BigDecimal length = new BigDecimal(kilometer);
        if (length.compareTo(BigDecimal.ZERO) >= 0) {
            // Work around for BigDecimal bug not returning exactly 0 when the answer is 0
            // This bug is fixed in Java 8, but Android still uses Java 7 if i'm not mistaken
            length = length.multiply(new BigDecimal("39370.07874015748031496062992126"))
     ...
     * @throws  NumberFormatException
     */
    public static String toFoot(@NonNull String kilometer, int roundingLength)
            throws NumberFormatException {
        ...
        if (length.compareTo(BigDecimal.ZERO) >= 0) {
            // Work around for BigDecimal bug not returning exactly 0 when the answer is 0
            // This bug is fixed in Java 8, but Android still uses Java 7 if i'm not mistaken
            length = length.multiply(new BigDecimal("3280.83989501312335958005249344"))
                    .setScale(decimalPlaces, BigDecimal.ROUND_HALF_UP);
            if (length.compareTo(BigDecimal.ZERO) == 0) {
                length = BigDecimal.ZERO;
            }
     ...
     * @throws  NumberFormatException
     */
    public static String toYard(@NonNull String kilometer, int roundingLength)
            throws NumberFormatException {
        ...
        if (length.compareTo(BigDecimal.ZERO) >= 0) {
            // Work around for BigDecimal bug not returning exactly 0 when the answer is 0
            // This bug is fixed in Java 8, but Android still uses Java 7 if i'm not mistaken
            length = length.multiply(new BigDecimal("1093.61329833770778652668416448"))
                    .setScale(decimalPlaces, BigDecimal.ROUND_HALF_UP);
            if (length.compareTo(BigDecimal.ZERO) == 0) {
                length = BigDecimal.ZERO;
            }
     ...
     * @throws  NumberFormatException
     */
    public static String toMile(@NonNull String kilometer, int roundingLength)
            throws NumberFormatException {
     ...
     * @throws  NumberFormatException
     */
    public static String toMillimeter(@NonNull String kilometer, int roundingLength)
            throws NumberFormatException {
        ...
        if (length.compareTo(BigDecimal.ZERO) >= 0) {
            // Work around for BigDecimal bug not returning exactly 0 when the answer is 0
            // This bug is fixed in Java 8, but Android still uses Java 7 if i'm not mistaken
            length = length.multiply(new BigDecimal("1000000"))
                    .setScale(decimalPlaces, BigDecimal.ROUND_HALF_UP);
            if (length.compareTo(BigDecimal.ZERO) == 0) {
                length = BigDecimal.ZERO;
            }
     ...
     * @throws  NumberFormatException
     */
    public static String toCentimeter(@NonNull String kilometer, int roundingLength)
            throws NumberFormatException {
        ...
        if (length.compareTo(BigDecimal.ZERO) >= 0) {
            // Work around for BigDecimal bug not returning exactly 0 when the answer is 0
            // This bug is fixed in Java 8, but Android still uses Java 7 if i'm not mistaken
            length = length.multiply(new BigDecimal("100000"))
                    .setScale(decimalPlaces, BigDecimal.ROUND_HALF_UP);
            if (length.compareTo(BigDecimal.ZERO) == 0) {
                length = BigDecimal.ZERO;
            }
     ...
     * @throws  NumberFormatException
     */
    public static String toMeter(@NonNull String kilometer, int roundingLength)
            throws NumberFormatException {
